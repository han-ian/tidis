...
impl RawClientWrapper {
    ...
    pub async fn redis_command(
        ...
        let key_ref = key.into();

        let router_key = KEY_ENCODER.encode_redis_router_key(table_id, &key_ref);
        debug!(
            LOGGER,
            "table_id: {}, router key: {}",
            table_id,
            hex::encode(&router_key)
        );
        debug!(LOGGER, "request: {}", String::from_utf8_lossy(&request));

        for i in 0..self.retries {
            match self
                .client
                .redis_command(table_id, cmd.clone(), router_key.clone(), request.clone())
                .await
            {
                Ok(Some(resp)) => {
                    return Ok(resp);
                ...
                Err(err) => {
                    TIKV_ERR_COUNTER
                        .with_label_values(&["raw_client_error"])
                        .inc();
                    debug!(LOGGER, "error to run redis command: {}", err);
                    if i > 1 {
                        error!(LOGGER, "error to run redis command: {}", err);
                    }
                    if self.error_retryable(&err) {
                        last_err.replace(err);
                        sleep(std::cmp::min(2 + i, 200)).await;
